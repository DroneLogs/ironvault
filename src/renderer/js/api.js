...
(function (IV) {
  ...
  IV.api = {
    ...
    captureStatus: () => call('capture.status'),
    captureRequest: (mode, credential, minutes) =>
      call('capture.request', { mode, credential, minutes }),
    captureRevoke: () => call('capture.revoke'),
    captureSetGuard: (guard) => call('capture.setGuard', { guard }),
    captureSetMinutes: (minutes) => call('capture.setMinutes', { minutes }),
    captureSetPassword: (password) => call('capture.setPassword', { password }),
    captureClearPassword: () => call('capture.clearPassword'),
    travelCandidates: (tag) => call('travel.candidates', { tag }),
    travelExport: (opts) => call('travel.export', opts || {}),
    lanStatus: () => call('lan.status'),
    lanEnable: (enabled) => call('lan.enable', { enabled }),
    lanBeginPairing: () => call('lan.beginPairing'),
    lanCancelPairing: () => call('lan.cancelPairing'),
    lanDiscover: () => call('lan.discover'),
    lanPair: (opts) => call('lan.pair', opts),
    lanForget: (id) => call('lan.forget', { id }),
    lanSetName: (name) => call('lan.setName', { name }),
    lanSync: (peerId) => call('lan.sync', { peerId }),
    browserStatus: () => call('browser.status'),
    browserEnable: (enabled) => call('browser.enable', { enabled }),
    browserForget: (id) => call('browser.forget', { id }),
    browserRegister: (browser, extensionId) => call('browser.register', { browser, extensionId }),
    browserUnregister: (browser) => call('browser.unregister', { browser }),
    browserReveal: () => call('browser.reveal'),
    aliasStatus: () => call('alias.status'),
    aliasVerify: (provider, apiKey) => call('alias.verify', { provider, apiKey }),
    aliasSaveKey: (provider, apiKey) => call('alias.saveKey', { provider, apiKey }),
    aliasClearKey: (provider) => call('alias.clearKey', { provider }),
    aliasCreate: (opts) => call('alias.create', opts || {}),
