 ...
 *
 * This is the honest version. An alias comes from SimpleLogin or addy.io, the
 * address exists, mail sent to it reaches you, and you can switch it off later
 * when it starts receiving spam. That is the whole point of an alias and it is
 * not something a password manager can fake.
 *
...
/**
 * Two providers rather than four done badly. These are the ones the privacy
 * audience actually uses, and each has a documented API that issues a real
 * address in one call.
 */
const PROVIDERS = {
  simplelogin: {
...
function choices() {
  return Object.values(PROVIDERS).map((p) => ({ key: p.key, name: p.name, keyUrl: p.keyUrl }));
}
